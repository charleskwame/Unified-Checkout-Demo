...
const loadCyberSourceSdk = (url, integrity) => new Promise((resolve, reject) => {
...
export default function UnifiedCheckout() {
  ...
  const startCheckout = async (event) => {
    ...
    setProcessing(true); setOpen(true); setError(""); setStatus("Loading checkout, please wait...");
    try {
      const payload = { targetOrigins: [window.location.origin], clientVersion: "1.0", country: "US", locale: "en_US", completeMandate: { type: "CAPTURE" }, data: { orderInformation: { amountDetails: { totalAmount: "50.00", currency: "USD" } } } };
      const sessionResponse = await fetch("/api/checkout-session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      ...
      const verifyResponse = await fetch("/api/verify-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ completeResponse: result }) });
  ...
  return <main className="checkout-shell">
    <section className="checkout-card" aria-busy={processing}>
      ...
      <div className="order-summary">
        ...
        <div className="price-row"><span>USD</span><strong>$50.00</strong></div>
      </div>
      <button className="proceed-button" type="button" disabled={processing} onClick={startCheckout}>{processing ? "Loading Checkout, Please Wait..." : "Proceed to Payment"}</button>
      <p className={`status${error ? " error" : ""}`} role={error ? "alert" : "status"}>{error || status}</p>
    </section>
